...
問2の機能まで実装した後、メソッドの抽出などのリファクタリングを行ったGreeterTestは次のようになっています（抜粋していますので、すべてのコードはGitHubを参照してください）。ポイントとしては、テストメソッドの中身を読んだ時にgreet()メソッドでテストしたいシナリオが明確になっているということです。また、このシナリオにおいては「どの時間範囲にマッチするのか」に関心があり、具体的な現在時刻の値については関心がありません（時間範囲をモックした時点でそうなっていました）。
